...
import com.osj.stockinfomation.databinding.FragmentTwelvemonthBinding;
import com.osj.stockinfomation.util.LogUtil;

import java.util.ArrayList;
import java.util.List;

public class TwelveMonthFragment extends Fragment implements View.OnClickListener , SwipeRefreshLayout.OnRefreshListener{
    FragmentTwelvemonthBinding binding;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        binding = DataBindingUtil.inflate(inflater,R.layout.fragment_twelvemonth, container, false);
        ...
        mPresenter.getProfitList(12, new CommonCallback.SingleObjectCallback<ProfitDAO>() {
            @Override
            public void onSuccess(ProfitDAO result) {
                for(int index = 0; index < result.getList().size(); index++){
                    profitList.add(new ProfitList(index,
